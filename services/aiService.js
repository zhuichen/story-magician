...
async function chatStream(messages, opts = {}, onDelta = () => {}) {
    ...
    res.data.on('data', (chunk) => {
      ...
      while ((idx = buffer.indexOf('\n\n')) !== -1) {
        ...
        if (DEBUG && debugCount < 5) {
          debugCount++;
          console.log(`[chatStream debug #${debugCount}] event=${eventName} type=${obj.type || ''} keys=${Object.keys(obj).join(',')} sample=${JSON.stringify(obj).slice(0, 400)}`);
        }
        ...
        const isDelta =
          type.endsWith('.delta') ||
          type === 'delta' ||
          (!type && (typeof obj.delta === 'string' || (obj.delta && typeof obj.delta === 'object')));
        if (!isDelta) continue;
        ...
        if (!delta && DEBUG) {
          console.log(`[chatStream] delta event but no text extracted: type=${type} keys=${Object.keys(obj).join(',')} sample=${JSON.stringify(obj).slice(0, 300)}`);
        }
  ...
  if (!full) {
    console.warn('[chatStream] 收到事件但未解析出文本，尝试非流式回退。设置 STREAM_DEBUG=true 可查看详细事件日志');
    const fallback = await chat(messages, opts);
    onDelta(fallback);
    return fallback;
  }
  if (DEBUG) {
    console.log(`[chatStream] 完成，full 长度=${full.length}，前 200 字: ${full.slice(0, 200)}`);
  }
  return full;
}
